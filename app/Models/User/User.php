...
class User extends Authenticatable implements MustVerifyEmail {
  ...
  public function getDisplayNameAttribute() {
    return ($this->is_banned  ? '<strike>'  : '') . '<a class="display-user" href="' . $this->url . '" style="' . (safe($this->rank?->color) ? 'color: #' . safe($this->rank?->color) . ';' : '') . ($this->is_deactivated ? 'opacity: 0.5;' : '') . '"><i class="' . ($this->rank->icon ? $this->rank->icon : 'fas fa-user') . ' mr-1" style="opacity: 50%;"></i>' . $this->name . '</a>' . ($this->is_banned ? '</strike>' : '');
  }
