  ...
  public function updateFaction($id, $user) {
    ...
    try {
      ...
      if ($id == null || (isset($old) && $faction->id != $old->id)) {
        $standing = $user
          ->getCurrencies(true)
          ->where('id', Settings::get('WE_faction_currency'))
          ->first();
        ...
        if (
          FactionRankMember::where('member_type', 'user')->where('member_id', $user->id)->first()
        ) {
          FactionRankMember::where('member_type', 'user')
  ...
  public function updateProfileImg($profile_img, $user) {
    ...
    try {
      ...
      if (config('lorekeeper.settings.username_change_cooldown')) {
        ...
        // different type
        $last_change = UserUpdateLog::where('user_id', $user->id)
          ->where('type', 'Username Change')
          ->orderBy('created_at', 'desc')
          ->first();
        if (
          $last_change &&
          $last_change->created_at->diffInDays(Carbon::now()) <
          config('lorekeeper.settings.username_change_cooldown')
        ) {
          throw new \Exception(
            'You must wait ' .
              config('lorekeeper.settings.username_change_cooldown') -
    ...
    try {
      if (!$user->is_banned) {
        ...
        foreach ($transfers as $transfer) {
          $characterManager->processTransferQueue(
            [
              'transfer' => $transfer,
              'action' => 'Reject',
              'reason' => ($transfer->sender_id == $user->id ? 'Sender' : 'Recipient') .
                ' has been banned from site activity.'
            ],
            $staff
          );
        }
        ...
        foreach ($submissions as $submission) {
          $submissionManager->rejectSubmission(
            [
              'submission' => $submission,
              'staff_comments' => 'User has been banned from site activity.'
            ],
            $staff
          );
        }
        ...
        $gallerySubmissions = GallerySubmission::where('user_id', $user->id)
          ->where('status', 'Pending')
          ->get();
        foreach ($gallerySubmissions as $submission) {
          $galleryManager->rejectSubmission($submission, $staff);
          $galleryManager->postStaffComments(
            $submission->id,
            ['staff_comments' => 'User has been banned from site activity.'],
            $staff
          );
        ...
        // 4. Design approvals
        $requests = CharacterDesignUpdate::where('user_id', $user->id)
        ...
        foreach ($trades as $trade) {
          $tradeManager->rejectTrade(
            ['trade' => $trade, 'reason' => 'User has been banned from site activity.'],
            $staff
          );
        }
      ...
      $user->settings->ban_reason =
        isset($data['ban_reason']) && $data['ban_reason'] ? $data['ban_reason'] : null;
      $user->settings->save();
