...
class PetManager extends Service {
  ...
  public function creditPet($sender, $recipient, $type, $data, $pet, $quantity, $variant_id = null, $evolution_id = null) {
    ...
    try {
      ...
      // If you're seeing this and there's a cleaner way to check for object type and also '->' please go ahead
      if ($type && !$this->createLog(
        isset($sender)
          ? (gettype($sender) == "object"
            ? $sender->id
            : $sender)
          : null,
        isset($recipient) ? $recipient->id : null,
        null,
        $type,
        $data['data'],
        isset($pet) ? $pet->id : null,
        $quantity
      )) {
