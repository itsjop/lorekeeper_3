        ...
        foreach ($items as $item) {
          ...
          if (
            $this->creditItem(
              ...
              $user,
              'Staff Grant',
              Arr::only($data, ['data', 'disallow_transfer', 'notes']),
              $item,
              $keyed_quantities[$item->id]
            )
          ) {
    ...
    try {
      foreach ($stacks as $key => $stack) {
        ...
        if (
          $recipient->logType == 'Character' &&
          isset($limit) &&
          ($ownedLimitedItems->pluck('count')->sum() >= $limit || $newOwnedLimit > $limit)
        ) {
          throw new \Exception(
            'One of the selected items exceeds the limit characters can own for its category.'
          );
        }
        ...
        $this->creditItem(
          $sender,
          $recipient,
          $sender->logType == 'User' ? 'User → Character Transfer' : 'Character → User Transfer',
          $stack->data,
          $stack->item,
          $quantity
        );
  ...
  public function transferStack($sender, $recipient, $stacks, $quantities) {
    ...
    try {
      foreach ($stacks as $key => $stack) {
        ...
        if (!$sender->hasAlias) {
          throw new \Exception(
            'You need to have a linked social media account before you can perform this action.'
          );
        }
        if (!$stack) {
      ...
      if ($owner->logType == 'User') {
        foreach ($stacks as $key => $stack) {
          ...
          if (
            $this->debitStack(
              $stack->user,
              $stack->user_id == $user->id ? 'User Deleted' : 'Staff Deleted',
              ['data' => $stack->user_id != $user->id ? 'Deleted by ' . $user->displayName : ''],
              $stack,
              $quantity
            )
          ) {
        ...
        foreach ($stacks as $key => $stack) {
          ...
          if (
            ...
            if (
              $stack->character->user_id != $user->id &&
              $stack->character->is_visible &&
              $stack->character->user_id
            ) {
    ...
    try {
      foreach ($stacks as $key => $stack) {
        ...
        if (isset($stack->item->data['resell']) && $stack->item->data['resell']) {
          ...
          if (
            !$currencyManager->creditCurrency(
              null,
              $oldUser,
              'Sold Item',
              'Sold ' . $stack->item->displayName . ' ×' . $quantity,
              $currency,
              $currencyQuantity
            )
          ) {
        ...
        if (
          $this->debitStack(
            $stack->user,
            $stack->user_id == $user->id ? 'Sold by User' : 'Sold by Staff',
            ['data' => $stack->user_id != $user->id ? 'Sold by ' . $user->displayName : ''],
            $stack,
            $quantity
          )
        ) {
  ...
  public function donateStack($user, $stacks, $quantities) {
    ...
    try {
      foreach ($stacks as $key => $stack) {
        ...
        $stock = UserItemDonation::where('stack_id', $stack->id)
          ->where('item_id', $stack->item->id)
          ->first();
        if ($stock) {
        ...
        if (
          $this->debitStack(
            $stack->user,
            $stack->user_id == $user->id ? 'Donated by User' : 'Donated by Staff',
            ['data' => $stack->user_id != $user->id ? 'Donated by ' . $user->displayName : ''],
            $stack,
            $quantity
          )
        ) {
