...
class CharacterManager extends Service {
  ...
  public function pullNumber($categoryId) {
    ...
    $number = 0; // First check if the number needs to be the overall next
    ...
    try {
      if ($isMyo) {
        ...
        $data['transformation_info'] =
          isset($data['transformation_info']) && $data['transformation_info'] ? $data['transformation_info'] : null;
  ...
  public function cropThumbnail($points, $characterImage, $isMyo = false) {
    ...
    if (
      Config::get('lorekeeper.settings.masterlist_image_format') != 'png' &&
      Config::get('lorekeeper.settings.masterlist_image_format') != null &&
      Config::get('lorekeeper.settings.masterlist_image_background') != null
    ) {
  ...
  public function updateCharacterProfile($data, $character, $user, $isAdmin = false) {
    ...
    try {
      ...
      if (isset($data['faction']) && $data['faction']) {
        ...
        if (isset($old) && $character->faction_id != $old) {
          ...
          if ($standing && $standing->quantity > 0) {
            if (
              !($debit = (new CurrencyManager())->debitCurrency(
                $character,
                null,
                'Changed Factions',
                null,
      ...
      if (isset($data['profession'])) {
        $character->profile->profession = $data['profession'];
      }
      if (isset($data['profession_id'])) {
        $character->profile->profession_id = $data['profession_id'];
      }
  ...
  public function createDesignUpdateRequest($character, $user, $image = null, $isImage = false) {
    ...
    try {
      ...
      if (CharacterDesignUpdate::where('character_id', $character->id)->active()->exists()) {
        throw new \Exception(
          'This ' .
            ($character->is_myo_slot ? 'MYO slot' : 'character') .
            ' already has an existing request. Please update that one, or delete it before creating a new one.'
        );
      }
      if (!$character->isAvailable) {
        throw new \Exception(
          'This ' .
            ($character->is_myo_slot ? 'MYO slot' : 'character') .
            ' is currently in an open trade or transfer. Please cancel the trade or transfer before creating a design update.'
        );
      }
      $data = [
        'user_id' => $user->id,
      ...
      $request = CharacterDesignUpdate::create($data); // If the character is not a MYO slot, make a copy of the previous image's traits
    ...
    try {
      ...
      if (!file_exists($request->thumbnailPath . '/' . $request->thumbnailFileName)) {
        ...
        if (!$isAdmin || ($isAdmin && isset($data['modify_thumbnail']))) {
          if (
            isset($data['use_cropper']) &&
            ($data['x0'] === null || $data['x1'] === null || $data['y0'] === null || $data['y1'] === null)
          ) {
      ...
      if (!$isAdmin || ($isAdmin && isset($data['modify_thumbnail']))) {
        $imageData = [];
        if (isset($data['use_cropper'])) {
          $imageData = Arr::only($data, ['species_id', 'subtype_id', 'rarity_id', 'use_cropper', 'x0', 'x1', 'y0', 'y1']);
          $imageData['use_cropper'] = isset($data['use_cropper']);
        }
        if (!$isAdmin && isset($data['image'])) {
          $imageData['extension'] = Config::get('lorekeeper.settings.masterlist_image_format')
            ? Config::get('lorekeeper.settings.masterlist_image_format')
  ...
  public function saveRequestAddons($data, $request) {
    ...
    try {
      ...
      if (isset($data['currency_id'])) {
        foreach ($data['currency_id'] as $holderKey => $currencyIds) {
          ...
          foreach ($currencyIds as $key => $currencyId) {
            ...
            if (
              !$currencyManager->debitCurrency($holder, null, null, null, $currency, $data['currency_quantity'][$holderKey][$key])
            ) {
      ...
      $currentFeatureIds = array_merge(
        $request->character->image->features->pluck('id')->toArray() ?? [],
        $request->getAttachedTraitIds()
      );
      if (count($currentFeatureIds) > 0) {
        $request
          ->features()
          ->whereNotIn('feature_id', array_merge($currentFeatureIds, $request->getAttachedTraitIds()))
          ->delete();
      } else {
  ...
  public function saveRequestFeatures($data, $request) {
    ...
    try {
      if (!($request->character->is_myo_slot && $request->character->image->species_id) && !isset($data['species_id'])) {
        throw new \Exception('Please select a ' . __('lorekeeper.species') . '.');
      }
      ...
      $species =
        $request->character->is_myo_slot && $request->character->image->species_id
        ? $request->character->image->species
        : Species::find($data['species_id']);
      if (isset($data['subtype_id']) && $data['subtype_id']) {
        $subtype =
          $request->character->is_myo_slot && $request->character->image->subtype_id
          ? $request->character->image->subtype
          : Subtype::find($data['subtype_id']);
      } else {
        $subtype = null;
      }
      ...
      if (!$species) {
        throw new \Exception('Invalid ' . __('lorekeeper.species') . ' selected.');
      }
      if ($subtype && $subtype->species_id != $species->id) {
        throw new \Exception(ucfirst(__('lorekeeper.subtype')) . ' does not match the ' . __('lorekeeper.species') . '.');
      }
      ...
      foreach ($data['feature_id'] as $key => $featureId) {
        ...
        if (!$request->isAttachedOrOnCharacter($featureId)) {
          continue;
        }
  ...
  public function approveRequest($data, $request, $user) {
    ...
    try {
          ...
          if (
            !$inventoryManager->debitStack(
              ...
              [
                'data' =>
                'Item used in ' .
                  ($request->character->is_myo_slot ? 'MYO design approval' : 'Character design update') .
                  ' (<a href="' .
                  $request->url .
                  '">#' .
                  $request->id .
      ...
      $extension =
        Config::get('lorekeeper.settings.masterlist_image_format') != null
        ? Config::get('lorekeeper.settings.masterlist_image_format')
        : $request->extension; // Create a new image with the request data
      $image = CharacterImage::create([
        ...
        'transformation_description' =>
        $request->character->is_myo_slot && isset($request->character->image->transformation_description)
          ? $request->character->image->transformation_description
      ...
      $request->artists()->update(['character_type' => 'Character', 'character_image_id' => $image->id]); // Add the compulsory features
      if ($request->character->is_myo_slot) {
      ...
      $request->rawFeatures()->update(['character_image_id' => $image->id, 'character_type' => 'Character']); // Make the image directory if it doesn't exist
      ...
      $this->processImage($image); // The thumbnail is already generated, so it can just be moved without processing
      File::move(
        $request->thumbnailPath . '/' . $request->thumbnailFileName,
        $image->thumbnailPath . '/' . $image->thumbnailFileName
      ...
      $request->character->sale_value = isset($data['sale_value']) ? $data['sale_value'] : 0; // Invalidate old image if desired
      ...
      $request->save(); // Add a log for the character and user
      $this->createLog(
        $user->id,
        null,
        $request->character->user_id,
        $request->character->user->url,
        $request->character->id,
        $request->update_type == 'MYO' ? 'MYO Design Approved' : 'Character Design Updated',
        '[#' . $image->id . ']',
        'character'
      );
      $this->createLog(
      ...
      $request->character->save(); // Set status to approved
      $request->staff_id = $user->id;
      $request->status = 'Approved';
      $request->save(); // Notify the user
