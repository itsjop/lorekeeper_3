<div id="defView" class="grid grid-4-col">
  ...
  @foreach ($items as $stackItem)
    @if ($totalItems < 4)
        ...
        $totalItems++; ?>
      <a
        href="#" class="grid ji-center inventory-stack text-center img" data-id="{{ $item->pivot->id }}"
        ...
        <img class="img-thumbnail" style="display: block" src="{{ $item->imageUrl }}" alt="{{ $item->name }}" />
        {{ $item->name }}
      </a>
