@extends('character.layout', ['componentName' => 'character/transfer', 'isMyo' => $character->is_myo_slot])
...
@section('profile-content')
  ...
  @if ($character->user_id == Auth::user()->id)
    ...
    @elseif($character->transferrable_at && $character->transferrable_at->isFuture())
      <p>This character is on transfer cooldown until <strong>{!! format_date($character->transferrable_at) !!}</strong>
        ({{ $character->transferrable_at->diffForHumans() }}). It cannot be transferred until then.</p>
    @elseif($transfer)
    ...
    @else
      <p>
        Transfers require the recipient to confirm that they want to receive the character. Before the recipient makes the
        confirmation, you may cancel the transfer, but cannot retrieve the character after it has been transferred.
        @if ($transfersQueue)
          Additionally, a mod will need to approve of the transfer. There may be a wait until the recipient receives the character,
          even after they have confirmed the transfer.
        @endif
      </p>
      @if ($cooldown)
        <p>
          After a character is transferred (transfer is accepted{{ $transfersQueue ? ' and approved' : '' }}), a cooldown of
          <strong>{{ $cooldown }}</strong> days will be applied. During this time, the character cannot be transferred to
          another person.
        </p>
      @endif
      ...
      <div class="form-group">
        ...
        {!! Form::select('recipient_id', $userOptions, old('recipient_id'), [
  ...
  @if (Auth::user()->hasPower('manage_characters'))
    ...
    <p>This will transfer the character automatically, without requiring the recipient to confirm the transfer. You may also
      transfer a character that is marked non-transferrable, or still under cooldown. Both the old and new owners will be notified
      of the transfer.</p>
    <p>Fill in either of the recipient fields - if transferring to an off-site user, leave the recipient field blank and vice versa.
    </p>
    {!! Form::open([
        'url' => $character->is_myo_slot
            ? 'admin/myo/' . $character->id . '/transfer'
            : 'admin/character/' . $character->slug . '/transfer'
    ]) !!}
    <div class="form-group">
      {!! Form::label('recipient_id', 'Recipient') !!}
      {!! Form::select('recipient_id', $userOptions, old('recipient_id'), [
