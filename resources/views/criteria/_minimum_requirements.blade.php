@if ($criterion)
  ...
  <div id="calc-{{ isset($id) ? $id : $criterion->id }}" class="ml-5 steps">
    ...
    @if (isset($isAdmin) && $isAdmin)
      ...
      <div class="text-right mb-3 row align-items-end">
        <div class="col" style="min-width:15em;">
          {!! Form::select(
              'criterion_currency_id[' . (isset($id) ? $id : $criterion->id) . ']',
              $reward_currencies,
    ...
    @else
      {!! Form::hidden(
          'criterion[' . (isset($id) ? $id : $criterion->id) . '][criterion_currency_id]',
          isset($criterion_currency) ? $criterion_currency : $criterion->currency_id
      ) !!}
    @endif
      ...
      <div class="form-group">
        ...
        @if ($step->type === 'input')
          {!! Form::number(
              ...
              $finalValues[$step->id] ?? null,
              ['class' => 'form-control', 'min' => isset($limitByMinReq) ? $minRequirements[$step->id] ?? null : null]
          ) !!}
        @elseif($step->type === 'options')
          ...
          {!! Form::select(
              ...
              $finalOptions,
              $finalValues[$step->id] ?? null,
              ['class' => 'form-control selectize', 'placeholder' => 'Select an Option']
          ) !!}
