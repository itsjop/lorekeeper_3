...
@section('admin-content')
  ...
  @else
    <p> Add an item to serve as a hunt target and be granted to users when they claim the target. While targets are identified internally and for admin purposes by their plain ID, they are only identified to members via a random string. This string is
      generated on creation-- links for placement around the site, etc. will be displayed once the target is created. </p>
  @endif
  ...
  @if ($target->id)
    <h3> Display Link </h3>
    <p> For convenience, here are links for placing targets around the site, etc. These links, being user-facing, make use of the randomized page ID for targets rather than the orderly (and thus predictable) internal ID. Links will need to be added to
      their respective destinations via source editing. Feel free to adjust the presentation of them, however. If the target item has an image, it displays only that; if it does not, it displays the item's name. </p>
