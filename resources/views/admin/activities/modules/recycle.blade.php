      ...
      @foreach ($settings->recyclables as $recyclable)
        <tr class="recyclable-row">
          <td>{!! Form::select(
              ...
              $recyclable->rewardable_type,
              ['class' => 'form-control recyclable-type', 'placeholder' => 'Select Recyclable Type']
          ) !!}</td>
          <td class="recyclable-row-select">
            @if ($recyclable->rewardable_type == 'Item')
              {!! Form::select('recyclable_id[]', $items, $recyclable->rewardable_id, [
                  'class' => 'form-control item-select selectize',
                  'placeholder' => 'Select Item'
              ]) !!}
            @elseif($recyclable->rewardable_type == 'ItemCategory')
              {!! Form::select('recyclable_id[]', $categories, $recyclable->rewardable_id, [
                  'class' => 'form-control item-select selectize',
