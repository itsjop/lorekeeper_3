...
@section('admin-content')
  ...
  @if ($award->id)
    <div class="card mb-3">
      ...
      <div class="card-body">
        <p>
          If you want this {{ __('awards.award') }} to be automatically unlocked by acquiring certain items, currencies, etc. add
          them here.
          <br />
          When a user has owned (at one point) all of these items, the {{ __('awards.award') }} will be distributed to them on their
          next visit to the {{ __('awards.awards') }} page
          <hr />
          If the progressions are changed after a user has claimed the {{ __('awards.award') }}, they will see the requirements
          they fulfilled alongside the current ones.
        </p>
        <hr />
    ...
    <div class="card mb-3">
      ...
      <div class="card-body">
        <p>
          ...
          <br />
          When a user receives an {{ __('awards.award') }} through obtaining all of the above items, they will also receive this
          reward.
        </p>
        <p>
...
@section('scripts')
  ...
  @include('js._reward_js', ['showLootTables' => false, 'showRaffles' => false, 'showRecipes' => true])
  <script>
    $(document).ready(function() {
      ...
      $('.delete-award-button').on('click', function(e) {
        e.preventDefault();
        loadModal("{{ url('admin/data/awards/delete') }}/{{ $award->id }}", 'Delete ' +
