@extends('admin.layout', ['componentName' => 'admin/worldexpansion/create-edit-location'])
...
@section('admin-content')
  ...
  {!! Form::open([
      'url' => $location->id ? 'admin/world/locations/edit/' . $location->id : 'admin/world/locations/create',
      'files' => true
  ]) !!}
    ...
    <div class="card-body">
      <div class="row mx-0 px-0">
        ...
        @if (isset($location->parent_id))
          <div class="form-group col-md px-0 pr-md-1">
            ...
            {!! Form::select('style', $location->displayStyles, isset($location->display_style) ? $location->display_style : null, [
      ...
      <div class="row mx-0 px-0">
        <div class="form-group col-12 col-md-6 px-0 pr-md-1">
          ...
          {!! Form::select('type_id', [0 => 'Choose a Location Type'] + $types, $location->type_id, [
              'class' => 'form-control selectize',
              'id' => 'type'
          ]) !!}
        </div>
        ...
        <div class="form-group col-12 col-md-6 px-0 px-md-1">
          ...
          {!! Form::select(
              'parent_id',
              [0 => 'Choose a Parent'] + $locations,
              isset($location->parent_id) ? $location->parent_id : null,
              ['class' => 'form-control selectize']
          ) !!}
        </div>
      </div>
      @if ($user_enabled || $ch_enabled)
        <div class=" mx-0 px-0 text-center">
          @if ($user_enabled)
            {!! Form::checkbox('user_home', 1, $location->id ? $location->is_user_home : 0, [
                'class' => 'form-check-input',
                'data-toggle' => 'toggle',
                'data-on' => 'Users Can Live Here',
                'data-off' => 'Users Cannot Live Here'
            ]) !!}
          @endif
          @if ($ch_enabled)
            {!! Form::checkbox('character_home', 1, $location->id ? $location->is_character_home : 0, [
                'class' => 'form-check-input',
                'data-toggle' => 'toggle',
                'data-on' => 'Characters Can Live Here',
                'data-off' => 'Characters Cannot Live Here'
            ]) !!}
          @endif
        </div>
      @endif
