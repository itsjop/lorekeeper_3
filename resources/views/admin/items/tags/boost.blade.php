<h3>Boost Item</h3>
<p>
  This is where you can specifiy which percentages during the pairing process this item boosts. The percentage is only applied if
  you chose a setting or rarity!
</p>
<hr />
<div id="settings" style="opacity: {{ isset($tag->getData()['rarity_id']) ? '0.5' : '1' }};">
  ...
  <p>
    ...
    <br>
    <strong>Example:</strong> If you set the setting to <strong>female inheritance</strong> and the percentage to
    <strong>75</strong>,
  ...
  <div class="row">
    <div class="col" id="percentage">
      {!! Form::number('setting_chance', $tag->getData()['setting_chance'] ?? 50, [
          'class' => 'form-control setting-percent',
          isset($tag->getData()['rarity_id']) ? 'disabled' : ''
      ]) !!}
    </div>
    <div class="col">
...
<div id="rarities" style="opacity: {{ isset($tag->getData()['setting']) ? '0.5' : '1' }};">
  <h3>Rarities</h3>
  <p>This will increase the chance of the chosen rarity being the character's rarity, and traits being chosen of this rarity, by
    the specified percentage.</p>
  {!! Form::label('Rarity') !!}
  <div class="row mb-3">
    <div class="col">
      {!! Form::number('rarity_chance', $tag->getData()['rarity_chance'] ?? 50, [
          'class' => 'form-control rarity-percent',
          isset($tag->getData()['setting']) ? 'disabled' : ''
      ]) !!}
    </div>
    <div class="col">
  ...
  $(document).ready(function() {
    $('#settingSelect').on('change', function() {
      ...
      if ($(this).val() == 1) {
        ...
        $('#percentage').append(
          '<div class="col info alert alert-info mt-2">This setting will remove the need for parents to be of the opposite sex, if set.</div>'
          );
      } else {
