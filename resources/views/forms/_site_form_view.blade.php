      ...
      @if ($question->options->count() > 0)
        @foreach ($question->options as $option)
          <div class="form-group mb-0">
            <label> {{ Form::radio($question->id, $option->id, $question->answers->where('user_id', $user->id)->where('submission_number', $number)->first()?->option_id == $option->id ? true : false, ['class' => 'mr-1', 'disabled' => 'disabled']) }}
              {{ $option->option }} </label>
          </div>
        @endforeach
      @else
