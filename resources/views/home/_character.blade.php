@if ($character)
  <div class="character-info" data-id="{{ $character->id }}">
    <img src="{{ $character->image->thumbnailUrl }}" class="mw-100" alt="Thumbnail for {{ $character->fullName }}" />
  </div>
  ...
  @if (!$character->is_visible && Auth::check() && Auth::user()->isStaff)
    <div class="text-danger character-info" data-id="0">
      <i class="fas fa-eye-slash mr-1"></i> Character hidden from public view.
    </div>
  @endif
@else
