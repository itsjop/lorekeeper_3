...
@section('home-content')
  ...
  @else
    {!! Form::open([
        'url' => $isClaim ? 'claims/new' : 'submissions/new',
        'id' => 'submissionForm',
    ...
    @if (!$isClaim)
      <div class="form-group">
        ...
        {!! Form::select('prompt_id', $prompts, Request::get('prompt_id'), [
    ...
    @if (!$isClaim)
      <div id="criterion-section" class="{{ Request::get('prompt_id') ? '' : 'hide' }}">
        <h2 class="mt-5">Criteria Rewards <button class="btn  btn-outline-info float-right add-calc" type="button">Add Criterion</a>
        </h2>
    ...
    @else
      <p>Note that any rewards added here are <u>in addition</u> to the default prompt rewards and caculated rewards. If you do not
        require any additional rewards, you can leave this blank.</p>
    @endif
    @if ($isClaim)
    ...
    <h2>Add-Ons</h2>
    <p>If your {{ $isClaim ? 'claim' : 'submission' }} consumes items, attach them here. Otherwise, this section can be left blank.
      These items will be removed from your inventory but refunded if your {{ $isClaim ? 'claim' : 'submission' }} is rejected.</p>
    ...
    >
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>This will submit the form and put it into the {{ $isClaim ? 'claims' : 'prompt' }} approval queue. You will not be
              able to edit the contents after the {{ $isClaim ? 'claim' : 'submission' }} has been made. Click the Confirm button to
              complete the {{ $isClaim ? 'claim' : 'submission' }}.</p>
            <div class="text-right">
              <a
                href="#"
                id="formSubmit"
